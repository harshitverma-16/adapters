...
from brokers.Zerodha.zerodha_adapter import ZerodhaAdapter
from brokers.Zerodha.zerodha_websocket import ZerodhaWebSocket
from common.broker_order_mapper import OrderLog

REDIS_HOST = "localhost"
REDIS_PORT = 6379
CH_BLITZ_REQUESTS = "adapter.channel"   # Incoming commands from Blitz
CH_ZERODHA_RESPONSES = "zerodha.response" # Outgoing Zerodha-specific responses
...
class ZerodhaConnector:
    def __init__(self, entity_id=None, creds=None):
        ...
        logging.info(f"[Connector] Connecting to Redis for entity {entity_id}...")
        self.redis = redis.Redis(host=REDIS_HOST, port=REDIS_PORT, decode_responses=True)
        self.pubsub = self.redis.pubsub()
        self.is_running = False
        logging.info("[Connector] Connected to Redis successfully.")
        ...
        # Automatically start WebSocket if access_token exists
        if self.access_token:
            logging.info(f"Access token found for entity {entity_id}, starting WebSocket...")
            self._start_websocket()
        else:
            logging.warning(f"No access token for entity {entity_id}. Manual login required.")
    ...
    def _process_message(self, raw_data):
        try:
            ...
            if target_entity and target_entity != self.entity_id:
                return
            if target_broker and target_broker != "Zerodha":
                return
            ...
            try:
                if action == "LOGIN":
                    ...
                    # Publish PENDING order to blitz.response
                    try:
                        order_id = result if isinstance(result, str) else result.get("order_id")
                        o = OrderLog()
                        o.ExchangeOrderId = order_id
    ...
    def _send_response_to_blitz(self, req_id, status, data, error):
        ...
        #  Print to terminal
        logging.info(
            "[BLITZ RESPONSE] req_id=%s | status=%s | error=%s | data=%s",
            req_id,
            status,
            error,
            data
...
# if __name__ == "__main__":
#     # In automated mode, pass creds from Redis
#     connector = ZerodhaConnector(entity_id="E123", creds={
#         "api_key": "2i4ayyawcrptt24h",
#         "api_secret": "2lxel09zt42jim5veokpgg6slrih2fpa",
#         "redirect_url": "http://localhost",
